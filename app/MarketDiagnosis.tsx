...
import styles from './MarketDiagnosis.module.css'
    ...
    <section className={styles.section} id="diagnosis">
      <div className={styles.intro}>
        ...
        <div className={styles.tabs}>
          <button className={mode === 'player' ? styles.active : ''} onClick={() => setMode('player')}>プレイヤー向け</button>
          <button className={mode === 'company' ? styles.active : ''} onClick={() => setMode('company')}>企業向け</button>
        </div>
      </div>
      ...
      <div className={styles.card}>
        ...
        <div className={styles.note}>※ 現時点では一般的なEC業務の難易度・経験年数・稼働量をもとにした参考値です。登録データが蓄積された後は、ECplayers内の実際の募集・希望報酬データを反映する予定です。</div>
      </div>
    </section>
  )
}
