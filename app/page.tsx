  ...
  return (
    <main>
      ...
      <section className="hero">
        ...
        <div className="searchBox" aria-label="スキル検索イメージ">
          <div className="searchIcon">⌕</div>
          <div className="searchCopy"><small>スキル・経験から探す</small><strong>楽天、Amazon、Shopify、広告運用...</strong></div>
          <a href="#players">探す →</a>
        </div>
        <div className="quickTags">{playerTags.slice(0,6).map(tag => <a href="#players" key={tag}>{tag}</a>)}</div>

        <div className="actions" id="register">
          <a className="primary" href="#players">3分で匿名プロフィールを作る <b>→</b></a>
          <a className="secondary" href="#jobs">3分で相談を掲載する <b>→</b></a>
        </div>
      ...
      <section className="section" id="players">
        <div className="sectionHeadRow">
          <div className="sectionHeading"><span>FIND PLAYERS</span><h2>会社名ではなく、<br />できることで探す。</h2><p>仕事に必要なスキル・経歴・稼働条件だけで探せる、匿名のECプレイヤーデータベース。</p></div>
          <a className="textLink" href="#">プレイヤーをすべて見る →</a>
        </div>
        ...
        <div className="cardGrid playerGrid">
          {players.map((p, index) => <article className="profileCard" key={p.id}>
            <div className="cardTop"><div className={`avatar a${index % 3}`}>P</div><div><small>PLAYER #{p.id}</small><strong>{p.meta}</strong></div><span className="availability">{p.availability}</span></div>
            <h3>{p.title}</h3><div className="tags">{p.tags.map(t => <span key={t}>{t}</span>)}</div><p>{p.text}</p>
            <button>この人に連絡してみる <span>→</span></button>
          </article>)}
        </div>
      ...
      <section className="sideBySide">
        <article><span>FOR PLAYERS</span><h2>会社員でも、<br />気軽に登録できる。</h2><p>名前や勤務先を公開せず、ECの経験と「こんな仕事なら話を聞きたい」だけ登録。今すぐ転職や独立を考えていなくても使えます。</p><a href="#register">匿名プロフィールを作る →</a></article>
        <article className="forCompany"><span>FOR COMPANIES</span><h2>採用前に、<br />まず話せる。</h2><p>求人票を作り込まなくても、困っていることと条件だけで掲載できます。気になるプレイヤーがいたら「連絡してみる」。</p><a href="#register">匿名で相談を掲載する →</a></article>
      </section>
      ...
      <section className="cta">
        <span>JOIN ECplayers</span><h2>ECの経験と仕事が、<br />ここに集まる。</h2><p>登録も、掲載も、マッチングも無料。まず3分で始められます。</p><div className="actions"><a className="primary light" href="#register">匿名プロフィールを作る →</a><a className="secondary dark" href="#register">相談を掲載する →</a></div>
      </section>
